...
// js agregados por lalo
#include "./webh/relays.js.gz.h"
#include "./webh/time.js.gz.h"
#include "./webh/alarmas.js.gz.h"
...
// -------------------------------------------------------------------
// Manejo de los archivos del servidor web relays.html
// -------------------------------------------------------------------
void handleRelays(AsyncWebServerRequest *request)
{
    ...
    AsyncWebServerResponse *response = request->beginResponse_P(200, dataTypeHTML, relays_html_gz, relays_html_gz_len);
    response->addHeader("Content-Encoding", "gzip");
    request->send(response);
}
// -------------------------------------------------------------------
// Manejo de los archivos del servidor web time.html
// -------------------------------------------------------------------
void handleTime(AsyncWebServerRequest *request)
{
    ...
    AsyncWebServerResponse *response = request->beginResponse_P(200, dataTypeHTML, time_html_gz, time_html_gz_len);
    response->addHeader("Content-Encoding", "gzip");
    request->send(response);
}
// -------------------------------------------------------------------
// Manejo de los archivos del servidor web alarmas.html
// -------------------------------------------------------------------
void handleAlarmas(AsyncWebServerRequest *request)
{
    if (security)
    {
        if (!request->authenticate(device_user, device_password))
    ...
    AsyncWebServerResponse *response = request->beginResponse_P(200, dataTypeHTML, alarmas_html_gz, alarmas_html_gz_len);
    response->addHeader("Content-Encoding", "gzip");
    request->send(response);
}
// -------------------------------------------------------------------
...
*/
//-----------------------------------------------------------------------------------------
void handleAlarmasJs(AsyncWebServerRequest *request) //////////////////////////////////////////////handleAlarmas
{
    if (security)
    {
        if (!request->authenticate(device_user, device_password))
    ...
    AsyncWebServerResponse *response = request->beginResponse_P(200, dataTypeJavaScripts, alarmas_js_gz, alarmas_js_gz_len);
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Access-Control-Max-Age", "600");
    request->send(response);
}
//-----------------------------------------------------------------------------------------
void handleTimeJs(AsyncWebServerRequest *request) //////////////////////////////////////////////handleTime
{
    if (security)
    {
        if (!request->authenticate(device_user, device_password))
    ...
    AsyncWebServerResponse *response = request->beginResponse_P(200, dataTypeJavaScripts, time_js_gz, time_js_gz_len);
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Access-Control-Max-Age", "600");
    request->send(response);
}
//----------------------------------------------------------------------------------------
void handleRelaysJs(AsyncWebServerRequest *request) //////////////////////////////////////////////handleRelays
{
    if (security)
    {
        if (!request->authenticate(device_user, device_password))
    ...
    AsyncWebServerResponse *response = request->beginResponse_P(200, dataTypeJavaScripts, relays_js_gz, relays_js_gz_len);
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Access-Control-Max-Age", "600");
    request->send(response);
}
